...
impl<N: Scalar> PhysicsModel<N> {
    ...
    pub fn new(x: N, y: N, z: N, w: N, h: N, d: N, vx: N, vy: N, vz: N, ax: N, ay: N, az: N, mass: N) -> PhysicsModel<N> {
        PhysicsModel {
    ...
    pub fn collide(&mut self, other: &mut PhysicsModel<N>, collision_vector_from_self_to_other: &CollisionVector<N>) {
        ...
        assert!( (self.momentum()+other.momentum() - total).abs() < N::cast(1)/N::cast(100000), "{} + {} = {} < {}, {} : {}", self.momentum(),other.momentum(), (self.momentum()+other.momentum()),total,self.mass,other.mass);
    }
...
impl<N: Scalar> BallPhysicsModel<N> {
    pub fn new(model: PhysicsModel<N>, radius: N) -> BallPhysicsModel<N> {
        BallPhysicsModel { physics_model: model, radius }
    }
    ...
    pub fn collision_vector(&self, other: &BallPhysicsModel<N>) -> Option<CollisionVector<N>> {
        ...
        if collision_depth >= N::cast(0) {
            let direction = glm::normalize(&(&other.physics_model.location - &self.physics_model.location)) * collision_depth;
