    ...
    fn load(input: impl AsRef<Path>) -> Result<(Vec<Self>, Vec<i32>), Error> {
        load(
            ...
            |vertex| {
                ...
                let texture_index = vertex
                    .1
                    .ok_or("texture UV index is mandatory! Fix you obj file")
                    .map_err(err_msg)?;
            ...
            |&(ver, tex), obj| {
                let ver_value = obj
                    ...
                    .position
                    .get(ver)
                    .ok_or("index pointing to nonexistent vertex")
                    .map_err(err_msg)?;
                let tex_value = obj
                    .data
                    .texture
                    .get(tex)
                    .ok_or("index pointing to nonexistent texture UV")
                    .map_err(err_msg)?;
        ...
        load(
            ...
            |vertex| {
                ...
                let texture_index = vertex
                    ...
                    .ok_or("texture UV index is mandatory! Fix you obj file")
                    .map_err(err_msg)?;
                let normal_index = vertex
                    .2
                    .ok_or("normal index is mandatory! Fix you obj file")
                    .map_err(err_msg)?;
            ...
            |&(ver, tex, nor), obj| {
                ...
                let nor_value = obj
                    ...
                    .normal
                    .get(nor)
                    .ok_or("index pointing to nonexistent normal")
                    .map_err(err_msg)?;
                let tex_value = obj
                    .data
                    .texture
...
impl LoadableFromObj for VertexTexNorTan {
    fn load(input: impl AsRef<Path>) -> Result<(Vec<Self>, Vec<i32>), Error> {
        VertexTexNor::load(input).and_then(|(ver_tex_nor, indices)| {
            compute_tangents(&ver_tex_nor, &indices)
                .map(|ver_tex_nor_tan| (ver_tex_nor_tan, indices))
        })
    }
}
...
pub fn load<T: Debug, K: Eq + Hash>(
    ...
    f: fn(&IndexTuple) -> Result<K, failure::Error>,
    k: fn(&K, &Obj) -> Result<T, failure::Error>,
) -> Result<(Vec<T>, Vec<i32>), failure::Error> {
    ...
    let triangles = &obj
        ...
        .objects
        .first()
        .ok_or("No objects in obj file")
        .map_err(err_msg)?
        .groups
        .first()
        .ok_or("No groups in obj file")
        .map_err(err_msg)?
        .polys;
    for triangle in triangles {
        if triangle.0.len() != 3 {
            return Err("Obj file contains non-triangle polygon").map_err(err_msg);
        }
...
pub fn compute_tangents(
    vertices: &[VertexTexNor],
    indices: &[i32],
) -> Result<Vec<VertexTexNorTan>, failure::Error> {
    ...
    fn orthogonalize(
        normal: &f32_f32_f32,
        tangent: &glm::TMat<f32, 3, 1>,
        bitangent: &glm::TMat<f32, 3, 1>,
    ) -> f32_f32_f32 {
    ...
    let converted_vertices = vertices
        .iter()
        .zip(tangents.iter())
        .zip(bitangents.iter())
        .map(|((v, &t), &b)| {
            VertexTexNorTan::convert(
                *v,
                orthogonalize(v.nor(), &t, &b),
                f32_f32_f32::new(b.x, b.y, b.z),
            )
        })
        .collect();
